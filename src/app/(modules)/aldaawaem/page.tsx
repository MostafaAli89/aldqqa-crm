      ...
      <div className="overflow-x-auto rounded-xl border border-border bg-card">
        <table className="w-full text-sm">
          <thead className="bg-muted text-muted-foreground">
            <tr>
              <th className="p-2 sm:p-3 text-right">البند</th>
              <th className="p-2 sm:p-3 text-right">الشهر</th>
          ...
          <tbody>
            {expenseSummary.map((e, i) => (
              <tr key={i} className="border-t border-border">
                <td className="p-2 sm:p-3">{e.category}</td>
                <td className="p-2 sm:p-3">{e.month}</td>
                <td className="p-2 sm:p-3">SAR {e.amount.toLocaleString('en-US')}</td>
              </tr>
            ))}
          </tbody>
        </table>
